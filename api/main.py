...
def read_one_order(order_id: int, db: Session = Depends(get_db)):
    ...
    if order is None:
        raise HTTPException(status_code = 404, detail = "No order found with that ID")
    return order
...
def read_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    ...
    if sandwich is None:
        raise HTTPException(status_code = 404, detail = "No sandwich found with that ID")
    return sandwich
...
def read_one_resource(resource_id: int, db: Session = Depends(get_db)):
    ...
    if resource is None:
        raise HTTPException(status_code = 404, detail = "No resource found with that ID")
    return resource
...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    ...
    if recipe is None:
        raise HTTPException(status_code = 404, detail = "No recipe found with that ID")
    return recipe
